...
class Game:
    ...
    def initialize(self): # i want to not hard coded the stuff below
        ...
        self.friendly_units_on_screen.append(Pipe(1, 'pipe', 'glory hole', 5, [Settings.GRID_BLOCK_SIZE*5 +Settings.ROW_OFFSET * 2, Settings.GRID_BLOCK_SIZE*6+Settings.ROW_OFFSET], 1, 1, 1, Settings.PIPE_1_IMAGE,{'L':True,'R':False,'U':False,'D':True}))
        self.friendly_units_on_screen.append(Nerd(1, 'nerd', 'glory hole', 5, [Settings.GRID_BLOCK_SIZE*0+Settings.ROW_OFFSET * 2, Settings.GRID_BLOCK_SIZE*6+Settings.ROW_OFFSET], 1, 1, 1, Settings.BASIC_NERD_IMAGE))
